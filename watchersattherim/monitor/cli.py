...
MONITOR_CONFIGS = ["monitor.ini", "~/.watchersattherim/monitor.ini"]
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    parser.add_argument("-c", "--config", default=None,
                        help="path to the INI config "
                             "(default: ./monitor.ini, then ~/.watchersattherim/monitor.ini)")
    ...
    parser.add_argument("-v", "--verbose", action="store_true",
                        help="echo decodes kept as observations (all raw decodes when [monitor] debug = true)")
    ...
    if args.identity:
        ...
        try:
            cfg_path = resolve_config(args.config, MONITOR_CONFIGS)
        except ConfigError:
            cfg_path = None     # identity needs no config, fall back to default storage
        storage_dir = _storage_dir(cfg_path, Storage().dir) if cfg_path else Storage().dir
        print_identity(os.path.join(os.path.expanduser(storage_dir), "identity"))
        return 0

    try:
        config = load(resolve_config(args.config, MONITOR_CONFIGS))
